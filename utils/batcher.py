...
class Example(object):
    ...
    def get_dec_inp_targ_seqs(self, sequence, max_len, start_id, stop_id):
        inp = [start_id] + sequence[:]+[stop_id]
        target = [start_id] +sequence[:]
...
class Batcher(object):
    ...
    def next_batch(self):
        ...
        if self._batch_queue.qsize() == 0:
            tf.compat.v1.logging.warning('Bucket input queue is empty when calling next_batch. '
    ...
    def fill_example_queue(self):
        input_gen = self.text_generator(self._data_path)
        ...
        while True:
            ...
            abstract_sentences = [sent.strip() for sent in abstract.split(' ')]  # Use the <s> and </s> tags in abstract to get a list of sentences.
            # print(abstract_sentences)
    ...
    def text_generator(self, path_train_seg):
        train_seg_df = pd.read_csv(path_train_seg, header=0)
        for article_text, abstract_text in zip(train_seg_df['正例文本'], train_seg_df['权利要求']):
