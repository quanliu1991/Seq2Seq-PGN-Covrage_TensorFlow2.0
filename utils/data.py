...
class Vocab(object):
  ...
  def __init__(self, vocab_file, max_size):
    ...
    vocab_w2i = np.loadtxt(config.path_vocab_w2i, dtype='str')
    for word_index in vocab_w2i:
      self._word_to_id[word_index[0]] = word_index[1]
      self._id_to_word[word_index[1]] = word_index[0]
      self._word.append(word_index[0])
      self._count = len(self._word)
    ...
    print("Finished constructing vocabulary of %i total words. Last word added: %s" % (self._count, self._id_to_word[str(self._count-1)]))

  def word2id(self, word):
    if word not in self._word_to_id:
      return int(self._word_to_id[UNKNOWN_TOKEN])
    return int(self._word_to_id[word])

  def id2word(self, word_id_int):
    word_id = str(word_id_int)
    if word_id not in self._id_to_word:
      raise ValueError('Id not found in vocab: %d' % word_id_int)
