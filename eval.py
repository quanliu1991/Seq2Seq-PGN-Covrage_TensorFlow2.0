...
def evaluate():
    ...
    model=Seq2Seq(config.eval_batch_size,embedding_matrix=embedding_matrix)
    ...
    if ckpt_manager.latest_checkpoint:
        ...
        logger.info('验证模型存档点加载自: {}'.format(ckpt_manager.latest_checkpoint))
    else:
        logger.info('无可加载的存档点')

    # 获取训练数据
    batcher = Batcher(config.path_seg_eval, vocab, mode='train',
                      batch_size=config.eval_batch_size, single_pass=False)

    time.sleep(10)
    # 训练模型
    # 输入：训练数据barcher，模型，词表，存档点，词向量矩阵
    eval_loss=eval_model(batcher, model=model,  ckpt_manager=ckpt_manager)
